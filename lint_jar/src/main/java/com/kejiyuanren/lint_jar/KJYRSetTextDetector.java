 ...
 */
public class KJYRSetTextDetector extends Detector implements UastScanner {

    private static final Implementation IMPLEMENTATION = new Implementation(
            KJYRSetTextDetector.class,
            Scope.JAVA_FILE_SCOPE);

    /** Constructing SimpleDateFormat without an explicit locale */
    public static final Issue SET_TEXT_I18N = Issue.create(
            "KJYR_SetTextI18n",
            "KJYR_TextView Internationalization",
    ...
    /** Constructs a new {@link KJYRSetTextDetector} */
    public KJYRSetTextDetector() {
    }
        ...
        if (node instanceof ULiteralExpression) {
            ...
            if (value instanceof String && value.toString().matches(WORD_PATTERN)) {
                context.report(SET_TEXT_I18N, node, context.getLocation(node),
                        "kejiyuanren : String literal in `setText` can not be translated. Use Android "
                                + "resources instead.");
            }
